" Vim indent file
" Language: Charly
" Maintainer: Leonard Schütz ( https://github.com/KCreate )
" Original File URL: https://github.com/pangloss/vim-javascript
...
setlocal indentexpr=GetCharlyIndent()
setlocal autoindent nolisp nosmartindent
setlocal indentkeys+=0],0)
" Testable with something like:
" vim  -eNs "+filetype plugin indent on" "+syntax on" "+set ft=charly" \
...
" au! Filetype charly let b:syng_str = '^\%(.*template\)\@!.*string\|special'
" au! Filetype charly let b:syng_strcom = '^\%(.*template\)\@!.*string\|comment\|regex\|special\|doc'

" Only define the function once.
if exists('*GetCharlyIndent')
  finish
endif
...
" GetCharlyIndent does to find the containing [[{(] (side-effects)
function s:SkipFunc()
  if s:top_col == 1
    throw 'out of bounds'
  endif
...
let s:opfirst = '^' . get(g:,'charly_opfirst',
      \ '\C\%([<>=,.?^%|/&]\|\([-:+]\)\1\@!\|\*\+\|!=\|in\%(stanceof\)\=\>\)')
let s:continuation = get(g:,'charly_continuation',
